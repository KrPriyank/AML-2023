...
sent = "What a beautiful day!"
threshold = 0.5
label,prop = score.score(sent,mlp,threshold)

class TestFunction:

    # check if score function is returning proper values
    def smoke_test(self):
        assert label!= None
        assert prop!= None
    ...
    # check if propensity lies in the range [0,1]
    def propensity_value(self):
        assert prop>=0 and prop<=1

    # if threshold is 0, prediction will become True
    def prop_test_0(self):
        label,prop = score.score(sent,mlp,0)
        assert label == 1

    # if threshold is 1, prediction will become False
    ...
    def spam_test(self):
        label,prop = score.score("You have won a free voucher of $10000, click on the link to redeem",mlp,threshold)
        assert label == 1

    # testing obvious ham
    def ham_test(self):
        label,prop=score.score("What a beautiful day!",mlp,threshold)
        assert label == 0
